...
# Modified by Jaegeon Jo in 2020.10
...
from autovc.datasets import MelDataset
from autovc.datasets import MelSCPDataset
...
def main():
    ...
    parser.add_argument("--feats-scp", default=None, type=str,
                        help="kaldi-style feats.scp file. "
                             "you need to specify either *-scp or rootdir.")
    parser.add_argument("--spkemb-scp", default=None, type=str,
                        help="kaldi-style spkemb.scp file. "
                             "you need to specify either *-scp or rootdir.")
    ...
    if (args.spkemb_scp is not None and args.rootdir is not None) or \
            (args.spkemb_scp is None and args.rootdir is None):
        raise ValueError("Please specify either --rootdir or --spkemb-scp.")
    ...
    for items in tqdm(dataset):
        if not args.skip_wav_copy:
            utt_id, audio, mel, spkemb = items
        else:
            utt_id, mel, spkemb = items
        ...
        if config["format"] == "hdf5":
            ...
            write_hdf5(os.path.join(args.dumpdir, f"{utt_id}.h5"),
                       "spkemb", spkemb.astype(np.float32))
            if not args.skip_wav_copy:
                write_hdf5(os.path.join(args.dumpdir, f"{utt_id}.h5"),
            ...
            np.save(os.path.join(args.dumpdir, f"{utt_id}-spkemb.npy"),
                    spkemb.astype(np.float32), allow_pickle=False)
            if not args.skip_wav_copy:
