...
class AudioMelDataset(Dataset):
    ...
    def __init__(self,
        ...
        Args:
            ...
            spkemb_query (str): Query to find speaker embedding files in root_dir.
            audio_load_fn (func): Function to load audio file.
            mel_load_fn (func): Function to load feature file.
            spkemb_load_fn (func): Function to load speaker embedding file.
        ...
        """
        # find all of audio, mel and spkemb files 
        audio_files = sorted(find_files(root_dir, audio_query))
        mel_files = sorted(find_files(root_dir, mel_query))
        spkemb_files = sorted(find_files(root_dir, spkemb_query))
        ...
        if audio_length_threshold is not None:
            ...
            spkemb_files = [spkemb_files[idx] for idx in idxs]
        ...
        assert len(audio_files) == len(spkemb_files), \
            f"Number of audio and spkemb files are different ({len(audio_files)} vs {len(spkemb_files)})."
        ...
        self.spkemb_load_fn = spkemb_load_fn
    ...
    def __getitem__(self, idx):
        ...
        spkemb = self.spkemb_load_fn(self.spkemb_files[idx])

        if self.return_utt_id:
            items = utt_id, audio, mel, spkemb
        else:
            items = audio, mel, spkemb
...
class MelDataset(Dataset):
    ...
    def __init__(self,
        ...
        assert len(mul_files) == len(spkemb_files), \
            f"Number of mel and spkemb files are different ({len(mel_files)} vs {len(spkemb_files)})."
